...
SELECT COUNT(*) FROM accident JOIN participated USING(report_no) WHERE accident_date>='2021-01-01' AND accident_date<='2021-12-31';
-- or
select COUNT(driver_id) from participated join accident using(report_no) where year(accident_date)="2021";
...
select driver_name,model from person 
join owns using(driver_id)
join car using(reg_no);
...
for each row 
begin
if (select count(driver_id) from participated where driver_id=NEW.driver_id = 2) then
